...
AVAILABLE_TIMES = ["10:00", "13:30", "16:00"]
INCLUDE_WEEKDAY = True
INCLUDE_WEEKEND = True
SPECIFIC_DATES = []  # 예: ["2026-08-01", "2026-08-02"]
        ...
        for date_obj in monitor_dates:
            ...
            for btn in buttons:
                ...
                if SPECIFIC_DATES:
                    ...
                    if is_weekday and not INCLUDE_WEEKDAY: continue
                    if not is_weekday and not INCLUDE_WEEKEND: continue
...
if __name__ == "__main__":
    print(f"▶️ [{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 감시 스크립트 실행")
    found = check_once()
    if found:
